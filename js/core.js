...
var stats, seq;
var mesh;
var curveA, curveB, line, warpTime, curvatures, tangents, pts;
...
function initWarp() {
    ...
    curvaturesA = computeCurvatures(curveA);
    curvaturesB = computeCurvatures(curveB);

    curvatures = computeCurvatures(curveA);
    tangents = computeTangents(curveA);
    pts = curveA;
...
function animateWarp() {
    ...
    for (cIdx=0; cIdx<curvatures.length; cIdx++) {
        curvatures[cIdx] = (1-warpTime)*curvaturesA[cIdx] + warpTime*curvaturesB[cIdx];
    }

    // Rebuild tangents from curvatures
    for (tIdx=1; tIdx<tangents.length; tIdx++) {
        angle = curvatures[tIdx];
        t0 = tangents[tIdx-1];
    ...
    for (pIdx=1; pIdx<pts.length; pIdx++) {
        pts[pIdx].copy(pts[pIdx-1]);
        pts[pIdx].add(tangents[pIdx-1]);
    }
    ...
    totalErr.subVectors(pts[0], pts[pts.length-1]);
    for (i=0; i<pts.length; i++) {
        t = i/(pts.length-1.0);
    ...
    for (i=0; i<line.geometry.vertices.length; i++) {
        line.geometry.vertices[i].copy(pts[i]);
    }
